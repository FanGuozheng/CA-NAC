...
def combine(runDirs,bmin_s,bmax_s,obmin,obmax, ispin, ikpt, potim,
            is_alle,is_reorder,is_real,iformat, new_nac=True):
    ...
    if iformat=='PYXAID' or iformat=='P':
        ...
        if new_nac:
            ...
            with open(nacre_filename, "ab") as f:
                np.savetxt(f, nac.real[:-1, :])
            with open(nacim_filename, "ab") as f:
                np.savetxt(f, nac.imag[:-1, :])
            with open(eig_out_filename, "ab") as f:
                np.savetxt(f, eig[:-1, :])
...
def nac_calc(runDirs, checking_dict, nproc=None, is_gamma=False,
             ...
             bmin_s=None, bmax_s=None,omin=None, omax=None,
             ikpt=1, ispin=1, icor=1, potim=1.0, new_nac=True):
        ...
        if completed_flag:
            ...
            if is_combine:
                ...
                combine(runDirs, bmin_s, bmax_s, ibmin, ibmax,
                        ispin, ikpt, potim,
                        is_alle, is_reorder, is_real, iformat, new_nac)
                if is_reorder:
